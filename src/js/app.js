...
function OptMouseMove(evt) {
  ...
  dndObj.ghostEl.classList.add('dragging'); //cursor
...
function OptDragEnd() {
  ...
  if(dndObj.ghostEl) {
    dndObj.ghostEl.classList.remove('dragging');
  }

  removeDel();

  if (dndObj.keepReserveEl && dndObj.keepReserveEl.parentElement) {
    const parent = dndObj.keepReserveEl.parentElement;
    parent.insertBefore(dndObj.dragEl, dndObj.keepReserveEl);
    parent.removeChild(dndObj.keepReserveEl);  // Удаляем элемент сразу после вставки
  ...
  dndObj.ghostEl = null;
  dndObj.dragEl = null;
  dndObj.keepReserveEl = document.createElement('div');  // Создаем новый элемент
  dndObj.keepReserveParent = null;
